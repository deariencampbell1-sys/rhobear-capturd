...
def main(shot_path: str) -> int:
    ...
    try:
        ...
        def adjusted(act_i: int) -> int | None:
            # act_i may reference an act step that either (a) never got an
            # engine stepIndex (demo.act timed out / was skip_unresolved'd,
            # so it's absent from act_to_engine), or (b) got one that was
            # then trimmed as a stray (present in act_to_engine but not in
            # kept). Either way there is no live engine step to point at --
            # warn and let the caller skip that directive instead of crashing
            # the whole render (content-dependent: only shows up on shot
            # lists whose act numbering drifted from what actually ran).
