...
Every flag is optional; with none it's a straight copy. Font defaults to Segoe UI
on Windows; override with --font <path>.
"""
...
FFMPEG = shutil.which("ffmpeg") or r"C:\Users\slang\sunsponge-capture\packaging\ffmpeg\ffmpeg.exe"
FFPROBE = shutil.which("ffprobe") or r"C:\Users\slang\sunsponge-capture\packaging\ffmpeg\ffprobe.exe"
...
def _title_card(text: str, wh: tuple[int, int], brand: str, font: str,
                seconds: float, out: Path) -> None:
    w, h = wh
    fs = max(40, int(h * 0.06))
    draw = (f"drawtext=fontfile='{_ff_font(font)}':text='{text}':"
...
def main() -> int:
    ...
    ap.add_argument("--font", default=r"C:\Windows\Fonts\segoeui.ttf")
    args = ap.parse_args()
